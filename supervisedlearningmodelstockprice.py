# -*- coding: utf-8 -*-
"""SupervisedLearningModelStockPrice.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/17GUj_CZCLWiGt1Lh-M4XH5bVI1IbsGPX
"""
...
#Y['MSFT_pred'] = Y['MSFT']
Y.name = Y.name+'_pred'
#stk_data_dd[['IBM', 'GOOGL']]
#stk_data_dd(['GOOGL', 'IBM'])
...
# define stock symbol and start/end dates
symbol = "AAPL"
...
decomposition = ARIMA(data['Close'], order=(3,1,1)).fit()
trend = decomposition.trend
seasonal = decomposition.seasonal
residual = decomposition.resid

# create a lagged DataFrame for the residual component
n_lags = 7  # choose number of lags
df_residual = pd.concat([residual.shift(i) for i in range(n_lags+1)], axis=1)
df_residual.columns = ['residual_t', 'residual_t-1', 'residual_t-2', 'residual_t-3', 'residual_t-4', 'residual_t-5', 'residual_t-6', 'residual_t-7']
df_residual.dropna(inplace=True)

# create target variable and train/test sets
X = df_residual.drop('residual_t', axis=1)
y = df_residual['residual_t']
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, shuffle=False)

# train linear regression model on residual component
...
print('Residual MSE: {:.4f}'.format(mse))

# add trend and seasonal components to final prediction
X_test_full = data['Close'].iloc[-n_lags-1:-1].values.reshape(1, -1)
X_test_full = np.concatenate([X_test_full, np.array([trend[-1], seasonal[-1]]).reshape(1, -1)], axis=1)
y_pred_final = model.predict(X_test)
y_pred_final = y_pred_final + trend[-1] + seasonal[-1]

# print final prediction
print('Final Prediction: {:.2f}'.format(y_pred_final[0]))
